...
function findBinary {
	...
	#BIN=$(grep "g++[^ ]*[ \t]*-o" $SINGLEREPORT.make | sed "s/^.*-o //;s/ build.*$//") Cambiado el 2/2/2024
	BIN=$(grep "g++[ \t]*[-Wall]*[ \t]*[-pedantic]*[ \t]*-o" $SINGLEREPORT.make | sed "s/^.*-o //;s/ build.*$//")
...
#		make -s test &>  $TESTS	
		...
		make clean
		rm $TESTREPORT
		timeout 90 make test | tee -a  $TESTS
