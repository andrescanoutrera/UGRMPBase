...
function doTest  {
    ...
    then 
        ...
        then
            ...
            #   cmake --build . &> ../${MAKEOUT}
            #cmake --build .  --clean-first &> ../${MAKEOUT}
            #cd ..
            cmake --build ${BUILD_DIRECTORY} &> ${MAKEOUT}
            #make &> ../${MAKEOUT} Otra posibilidad para obtener ejecutable si usaramos Makefile generator en proyecto CLion
        ...
        if [ "$IDE" == "CLION" ]
        then
            # This should be used if Makefile Generator is used in the CLion project
            #BINARY=cmake-build-debug/$(grep "Built target " $MAKEOUT | sed "s/^.*Built target //")
            # This should be used if Ninja Generator is used in the CLion project, but It also could be used with Makefile Generator
            #BINARY=cmake-build-debug/$(grep "CMAKE_PROJECT_NAME:STATIC=" cmake-build-debug/CMakeCache.txt | sed "s/CMAKE_PROJECT_NAME:STATIC=//")
            BINARY=${BUILD_DIRECTORY}/$(grep "CMAKE_PROJECT_NAME:STATIC=" ${BUILD_DIRECTORY}/CMakeCache.txt | sed "s/CMAKE_PROJECT_NAME:STATIC=//")
        elif [ "$IDE" == "VSCODE" ]
        then
            BINARY=${BUILD_DIRECTORY}/$(grep "CMAKE_PROJECT_NAME:STATIC=" ${BUILD_DIRECTORY}/CMakeCache.txt | sed "s/CMAKE_PROJECT_NAME:STATIC=//")
        else # [ "$IDE" == "NETBEANS" ]
        ...
        # Remove empty/blank lines from $TESTS_FOLDER/.real and  $TESTS_FOLDER/.due$k (taken from https://serverfault.com/questions/252921/how-to-remove-empty-blank-lines-from-a-file-in-unix-including-spaces)
        #REAL_OUTPUT=$(sed -i '/^$/d' $TESTS_FOLDER/.real$k)
        #DUE_OUTPUT=$(sed -i '/^$/d' $TESTS_FOLDER/.due$k)
