...
public:
     ...
     */
    int findLocation(const Location &) const;
     ...
     */
    const Location &at(int pos) const; //consulta, cuando es un metodo por referencia se hace const al principio y al final
     ...
     */
    Location &at(int pos); //modificacion


    //Si en main hago uno.at(0) = loc; paso los datos de loc a la matriz, entonces se hace el de modificacion
    //pero si hago loc=uno.at(1); paso a loc los datos que hay en la matriz en la posicion 1 a loc
